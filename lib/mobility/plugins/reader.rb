          ...
          names.each do |name|
            class_eval <<-EOM, __FILE__, __LINE__ + 1
              def #{name}(locale: nil, **options)
                #{Reader.setup_source}
                mobility_backends[:#{name}].read(locale, **options) || mobility_backends[:#{name}].read(I18n.default_locale, **options)
              end
            EOM
            class_eval <<-EOM, __FILE__, __LINE__ + 1
              def #{name}?(locale: nil, **options)
                #{Reader.setup_source}
                mobility_backends[:#{name}].present?(locale, **options) || mobility_backends[:#{name}].read(I18n.default_locale, **options)
              end
            EOM
          end
        end
